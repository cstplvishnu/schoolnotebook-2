		...
		<div class="row">
				...
				<div class="col-sm-6">
                                 <fieldset class='form-group'> 
                                  {{ Form::label('exampleInputEmail1', getphrase('is_parent_account_available')) }}
                        <div class="m-b-sm">
		                <div class="btn-group toggle-tick">
		                  <label class="btn btn-sm btn-info ss-radio-no active" for="available"><input type="radio" checked="checked" id="available" name="account" value="1" ng-model="account_available" ng-init="account_available=1; accountAvailable(1);" ng-click="accountAvailable(1)"><i class="fa fa-check text-active"></i>{{getPhrase('Yes')}}
		                  </label>
		                  <label class="btn btn-sm btn-success ss-radio-no" for="not_available"><input type="radio" id="not_available" name="account" value="0" ng-model="account_not_available" ng-click="accountAvailable(0)">
		                   <i class="fa fa-check text-active"></i>{{getPhrase('No')}}
		                  </label>
		                </div>
		              </div>
                       </fieldset> 
