    ...
    */
     ...
     { 
      ...
      if(!checkRole(getUserGrade(2)))
        ...
        $expected_roles = ['owner','admin','staff','student','librarian','assistant_librarian', 'parent','clerk', 'users'];
    ...
    { 
        ...
        if($slug=='users')
        {
            ...
            ->select(['image','users.name','email','roles.display_name','roles.name as role_name','login_enabled','role_id',
        ...
        elseif($slug =='student') 
        {
            ...
            ->where('roles.id', '=', $role)
            ->select(['image','users.id','users.name', 'students.roll_no','courses.course_title','students.current_year','students.current_semister', 'email','roles.name as role_name','login_enabled','role_id','users.slug as slug', 'users.updated_at','courses.course_dueration','courses.is_having_semister'])
          ...
          elseif($slug =='staff') 
            ...
            ->select(['image','users.name', 'staff.staff_id','staff.job_title','courses.course_title', 'email','roles.name as role_name','login_enabled','role_id','users.slug as slug', 'users.updated_at','users.status','staff.user_id'])
            ...
            $records = User::join('roles', 'users.role_id', '=', 'roles.id')
            ->where('roles.id', '=', $role)
            ->select(['image','users.name',   'email','roles.name as role_name','login_enabled','role_id','users.slug as slug', 'users.updated_at'])
        ...
        ->addColumn('action', function ($records) {
          ...
          $link_data = '<p id="social-buttons">
                      ...
                      <a href="'.URL_USERS_EDIT.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("edit").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-pencil"></i></a>';
                    ...
                    if($records->role_name == 'student') {
                        ...
                        $link_data .= '
                           <a href="'.URL_STUDENT_EDIT_PROFILE.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("admission_details").'"  class="btn btn-sm btn-icon btn-success"><i class="fa fa-user" aria-hidden="true"></i></a>

                            <a href="'.URL_USER_DETAILS.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("profile").'"  class="btn btn-sm btn-icon btn-default"><i class="fa fa-university" aria-hidden="true"></i></a>';
                      }
                    ...
                    if($records->role_name == 'staff') {
                      ...
                      if(!$records->status)
                        ...
                        $link_data .= '
                           <a href="'.URL_STAFF_EDIT_PROFILE.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("edit_details").'"  class="btn btn-sm btn-icon btn-primary"><i class="fa fa-user" aria-hidden="true"></i></a>

                           <a href="javascript:void(0);" onclick="changeStatus(\''.$records->slug.'\',\''.$records->status.'\',\''.$records->user_id.'\');" data-toggle="tooltip" data-placement="auto" title="'.$status_name.'"  class="btn btn-sm btn-icon btn-warning"><i class="fa fa-star" aria-hidden="true"></i></a>';
                      }
         ...
         ->editColumn('name', function($records) {
          ...
          if($records->role_id==5){
             
             $data   = Student::where('user_id','=',$records->id)->first();

             if($data->roll_no!=null)
               return ' <strong>'.ucfirst($records->name).'</strong><p><strong>'.getPhrase('roll_no').'</strong> :'
              .$data->roll_no.'</p>'; 
         ...
         return ' <strong>'.ucfirst($records->name).'</strong>';
       }
         ...
         ->editColumn('image', function($records){
          $role = getRoleData($records->role_id);
            if($role =='student'){
            return '<a href="'.URL_USER_DETAILS.$records->slug.'"><img src="'.getProfilePath($records->image).'"  class="img-circle" /></a>';
          }

            elseif ($role=='staff') {
              return '<a href="'.URL_STAFF_DETAILS.$records->slug.'"><img src="'.getProfilePath($records->image).'"  class="img-circle" /></a>';
            }
            else{
    ...
    public function getStaffInactiveList($slug='staff')
    {
        ...
        DB::statement(DB::raw('set @rownum=0'));
          ...
          $records = User::join('roles', 'users.role_id', '=', 'roles.id')
            ...
            ->where('roles.id', '=', $role)
            ->where('users.status', '=',0)
            ->select(['image','users.name',  'staff.staff_id','staff.job_title','courses.course_title', 'email','roles.name as role_name','login_enabled','role_id','users.slug as slug', 'users.updated_at','users.status'])
        ...
        ->addColumn('action', function ($records) {
            ...
            return '<a href="javascript:void(0);" onclick="changeStatus(\''.$records->slug.'\',\''.$records->status.'\');" ><button type="button" class="btn btn-info">'
                    .$status_name.'</button></a>';
            })
        ->editColumn('name', function($records) {
          
        
              return '<a href="'.URL_STAFF_DETAILS.$records->slug.'">'.ucfirst($records->name).'</a>';
          })
         ->editColumn('image', function($records){
            return '<img src="'.getProfilePath($records->image).'" class="img-circle" />';
        })
...
/**
    ...
    public function changeStatus(Request $request)
    {
      ...
      $record->save(); 
       ...
       flash('Oops...','this staff is need to return books in library', 'info');
          return redirect(URL_USERS."staff");
     ...
     public function store(Request $request )
     {
        ...
        if($user_type == 'staff')
        {
          ...
          DB::commit();
          flash('success','record_added_successfully', 'info');
    ...
    catch(Exception $ex)
    {
       ...
       if(getSetting('show_foreign_key_constraint','module'))
       {
          ...
          flash('oops...!', $ex->getMessage(), 'error');
           ...
           flash('oops...!','please_check_your_email_master_settings', 'info');
       }
    }
    ...
    {   
        ...
        if(empty($studentdata->academic_id)||empty($studentdata->course_parent_id)||empty($studentdata->course_id)){
          ...
          flash('Oops...!','Your Admission Details Are Not Updated', 'info');
    ...
    */
     ...
     public function getlibraryhistory($user_id)
     {
        ...
        ->editColumn('master_asset_id', function($records) {
          ...
          $data ='';
          $data = '<strong>'.$record->title.'</strong>';
          return $data;
     ...
     /**
     This Methos Display The Student Profile
     **/
    public function details($slug)
    {
        ...
        $record     = User::where('slug', $slug)->get()->first();
         if($record==null){
          flash('Ooops','Undefined User','info');
          return back();
         }
         if($record->role_id!=5){
          flash('Ooops','You Have No Permission To Access','info');
          return back();
         }
      ...
      $student_details = Student::where('user_id','=',$user_id)->first();
       ...
       if($isValid = $this->isValidRecord($student_details))
         return redirect($isValid);
    ...
    **/
    public function feeShedulesView($id)
    {

      if($id==''){
       
       flash('Ooops','student_is_not_avilable','info');
       return back();
      ...
      $allFeeCategories    = App\FeeScheduleParticular::getAllCategories($id);
         ...
         if(count($allFeeCategories)==0){
          flash('Oops','schedules_are_not_updated','info');
          return back();
         }
      ...
      $feecategory_details =  App\FeeCategory::getCategory($student_details);
         ...
         if($isValid = $this->isValidRecord($feecategory_details))
         return redirect($isValid);
      ...
      $data['active_class']  = 'users';
      if(checkRole(getUserGrade(5))){
         $data['active_class']  = 'users-1';
      }
      $data['sub_active_class']   = 'fee_schedules';
    ...
    **/
    public function feePaidHistoryView($id)
    {
      ...
      if($id==''){
       ...
       $allFeeCategories    = App\FeeScheduleParticular::getAllCategories($id);
          if(count($allFeeCategories)==0){
          flash('Oops','no_data_available','info');
          return back();
         }
      ...
      $data['sub_active_class']   = 'fee_history';
     ...
     public function addUser($users)
     {
      foreach($users as $request) {
        ...
        $user->roles()->attach($user->role_id);
          ...
          $student = new App\Student();
          $student->admission_no = $student->preparestudentID($user->id);
          $student->roll_no = $student->prepareRollNo($user->id, $request->academic_id, $request->course_parent_id, $request->course_id, $current_year, $current_semister);
  ...
  public function settings($slug)
  {
     ...
     if(\Auth::user()->id == $record->id)
      ...
      if(!$UserOwnAccount)  {
       ...
       $data['layout']       = getLayout();
       $data['active_class'] = 'users_settings';
       $data['sub_active_class']   = 'users';
...
{  
    ...
    $records   = User::join('students','students.user_id','=','users.id')
                      ->join('courses','courses.id','=','students.course_id')
                       ->where('name','like',$text.'%')
                       ->select(['image','users.slug','users.email','students.roll_no','course_title','users.name'])
                       ->get();
