...
class LibraryIssuesController extends Controller
{
    ...
    {  
         ...
         if(!checkRole(getUserGrade(9)))
      {
        ...
        if($role_name=='student'){
             ...
             $data['sub_active_class']       = 'library_students';
         }

         else{
          
             $data['sub_active_class']       = 'library_staff';
         }
        ...
        $data['title']              = getPhrase('student_users');
          ...
          if($role_name =='staff'){
        $data['title']              = getPhrase('faculty_users');
         }
        
        if($role_name == 'student')
        $data['table_columns']      = ['image','roll_no',  'first_name', 'last_name', 'email'];
        
        else if($role_name == 'staff')
        $data['table_columns']      = ['image','staff_id',  'first_name', 'last_name', 'email'];
    ...
    public function getUsers($role_name='student')
    {
         if(!checkRole(getUserGrade(9)))
      {
        ...
        if($role_name == 'student') {
         ...
         ->select(['users.image','roll_no','first_name','last_name','users.email','users.slug']);
        ...
        ->editColumn('image', function($records){
           ...
           ->editColumn('roll_no', function($records){
            return '<strong><a href="'.URL_LIBRARY_ISSUES.$records->slug.'">'.$records->roll_no.'</strong></a>';
        })
         ->removeColumn('slug')
         ->make(true);
         ...
         ->where('status','!=',0)
            ->select(['users.image','staff.staff_id','staff.first_name','staff.last_name','users.email','users.slug' ]);
        ...
        ->editColumn('image', function($records){
           ...
           ->editColumn('staff_id', function($records){
            return '<strong><a href="'.URL_LIBRARY_ISSUES.$records->slug.'">'.$records->staff_id.'</strong></a>';
        })
    ...
    public function getMasterDetails(Request $request)
    {
    	...
    	if($asset){
	    	...
	    	$details  = '<li class="list-group-item"><span>'.getPhrase('title').'</span> :<strong>'.$asset->title.'</strong></li>';
	    	$details .= '<li class="list-group-item"><span>ISBN</span> :<strong>'.$asset->isbn.'</strong></li>';
	    	$details .= '<li class="list-group-item"><span>'.getPhrase('edition').'</span> :<strong>'.$asset->edition.'</strong></li>';
	    	$details .= '<li class="list-group-item"><span>'.getPhrase('author').'</span>: <strong>'.$author->author.'</strong></li>';
	    	$details .= '<li class="list-group-item"><span>'.getPhrase('publisher').'</span>: <strong>'.$publisher->publisher.'</strong></li>';
	    	$details .= '<li class="list-group-item"><span>'.getPhrase('total').'</span>: <strong>'.$asset->total_assets_count.'</strong></li>';
	    	$details .= '<li class="list-group-item"><span>'.getPhrase('available').'</span>: <strong>'.$asset->total_assets_available.'</strong></li>';
    	}
    	...
    	if($instance)
    	{
    		$instance_details .= '<li class="list-group-item"><span>'.getPhrase('category').'</span> :<strong>'.ucfirst($asset_type->asset_type).'</strong></li>';
    		$instance_details .= '<li class="list-group-item"><span>'.getPhrase('reference_no.').'</span> :<strong>'.$instance->asset_no.'</strong></li>';
    		$instance_details .= '<li class="list-group-item"><span>'.getPhrase('type').'</span> :<strong>'.ucfirst($instance->asset_type).'</strong></li>';
    		$instance_details .= '<li class="list-group-item"><span>'.getPhrase('status').'</span> :<strong>'.ucfirst($instance->status).'</strong></li>';
    		$instance_details .= '<li class="list-group-item"><span>'.getPhrase('eligiblity').'</span> :<strong>'.$eligiblity->message.'</strong></li>';
    	...
    	if($eligiblity->is_eligible)
    	{
    		$button = '<button onClick="alert(/"hello jack/");" class="btn btn-info">'.getPhrase('issue').'</button>
						';
    	}
		...
		$data['image'] = '<img class="img-responsive img-circle" src="'.$imagepath.'" width="300px"/>';
