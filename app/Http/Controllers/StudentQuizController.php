...
class StudentQuizController extends Controller
{
    ...
    public function index()
    {
      ...
      if(checkRole(getUserGrade(2)))
      {
        ...
        $data['active_class']       = 'exams';
        $data['sub_active_class']   = 'categories';
    ...
    public function instructions($slug)
    {
      ...
      if($student_details->roll_no==null){
        flash('Oops','Student Admission Details Are Not Updated','warning');
        return back();
      } 
      ...
      if(!checkRole(['student']))
      {
    	...
    	$data['record']       	  = $record;
      $data['active_class']     = 'exams';
      $data['sub_active_class'] = 'exams';
    	$data['layout']           = getLayout();
      $data['title']            = $record->title;
      $data['block_navigation'] = TRUE;
      $data['block_sidebar']    = FALSE;
    ...
    public function getDatatable($slug = '')
    {
        ...
        ->addColumn('action', function ($records) {
            ...
            if(!checkRole(['student']))
              ...
              return '<p id="social-buttons">
                      ...
                      <a href="'.URL_PAYMENTS_CHECKOUT.'exam/'.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("buy_now").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-shopping-cart"></i></a></p>';

              else 
                return '-';
            return '<p id="social-buttons">

                      <a href="'.URL_STUDENT_TAKE_EXAM.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("take_exam").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-pencil"></i></a></p>';
        ...
        {
            return round($records->total_marks);
        })
        ->editColumn('title', function($records) 
        {
          if(!checkRole(['student'])) {
            if($records->is_paid) {
            return '<strong><a href="'.URL_PAYMENTS_CHECKOUT.'exam/'.$records->slug.'">'.$records->title.'</a></strong>';
            }
          ...
          return '<strong><a onClick="showInstructions(\''.URL_STUDENT_TAKE_EXAM.$records->slug.'\')" href="javascript:void(0);">'.$records->title.'</a></strong>';
    ...
    public function getScheduledExams($slug)
    {
      ...
      $data['user']             = $user_record;
      $data['active_class']     = 'exams';
      $data['sub_active_class'] = 'scheduled_exams';
      $data['title']            = getphrase('scheduled_exams');
      $data['layout']           = getLayout();
      return view('student.exams.list', $data);      
    }
    ...
    {  
      ...
      $data['sub_active_class']   = 'exam_attempt_history';
    ...
    public function getExamAttemptsData($slug, $exam_slug = '')
    {
         ...
         ->addColumn('action', function($records)
        {
          ...
          $options = '<p id="social-buttons">
                        
                           <a href="'.URL_RESULTS_VIEW_ANSWERS.$records->slug.'/'.$records->resultsslug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("view_answers").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-eye"></i></a>';
                      
                           $certificate_link = '';
                           if(getSetting('certificate','module'))
                           {
                            $certificate_link = '&nbsp;<a href="'.URL_GENERATE_CERTIFICATE.$records->resultsslug.'" target="_blank" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("generate_certificate").'"  class="btn btn-sm btn-icon btn-success" ><i class="fa fa-certificate"></i></a>';
                           }
        ...
        ->editColumn('title', function($records)
        {
          ...
          return '<strong><a href="'.URL_STUDENT_EXAM_ANALYSIS_BYSUBJECT.$user->slug.'/'.$records->slug.'/'.$records->resultsslug.'">'.ucfirst($records->title).'</a></strong>';
        })
    ...
    public function getExamAnalysisData($slug)
    {
         ...
         ->editColumn('title', function($records)
        {
          ...
          return '<strong><a href="'.URL_STUDENT_EXAM_ATTEMPTS.$user->slug.'/'.$records->slug.'"">'.$records->title.'</a></strong>';
        })
        ->editColumn('is_paid', function($records)
        {
            return ($records->is_paid) ? '<span class="label label-primary">'.getPhrase('paid') .'</span>' : '<span class="label label-success">'.getPhrase('free').'</span>';
    ...
    public function overallSubjectAnalysis($slug)
    {
      ...
      if(!isEligible($slug))
       ...
       if(!$records)
        {
          flash('Ooops..!','No Records available', 'info');                
          return back();
        }
      ...
      $data['active_class']       = 'analysis';
      $data['sub_active_class']   = 'by_subject';
