        ...
        if(!checkRole(getUserGrade(2)))
      {
        ...
        return back();
         ...
         $records = LmsCategory::select([   
         	'image','category','id','slug'])->orderBy('updated_at', 'desc');
        $this->setSettings();
        ...
        ->addColumn('action', function ($records) {
           ...
           $link_data = '<p id="social-buttons">
                      ...
                      <a href="'.URL_LMS_CATEGORIES_EDIT.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("edit").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-pencil"></i></a>';
        ...
        ->editColumn('category',function($records){
           ...
           return '<strong>'.$records->category.'</strong>';

        })
        ->editColumn('image', function($records){
            $image = '<img src="'.IMAGE_PATH_UPLOAD_LMS_DEFAULT.'" height="50" width="50" class="img-circle" />';
            if($records->image)
            $image = '<img src="'.IMAGE_PATH_UPLOAD_LMS_CATEGORIES.$records->image.'" height="70" width="70"  class="img-circle" />';
        return $image;
        })
        ->make();
    }
