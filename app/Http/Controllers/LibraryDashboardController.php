...
class LibraryDashboardController extends Controller
{
    ...
    { 
      ...
      $data['active_class']       = 'library';
      $data['sub_active_class']   = 'student_return';
      }
      else{
    ...
    public function staffbooks()
    {
      if(!checkRole(getUserGrade(15))){
      $data['active_class']       = 'library';
      $data['sub_active_class']   = 'staff_return';
      }
      else{
      $data['active_class']       = 'staff-return';
      $data['sub_active_class']   = 'staff_return';
      }
      $data['title']              = getPhrase('faculty_book_returns');
      $data['layout']             = getLayout();
 ...
 */
    ...
    $link_data = '<a href="javascript:void(0);" onclick="changeStatus(\''.$records->user_id.'\',\''.$records->id.'\',\''.$records->library_instance_id.'\',\''.$records->master_asset_id.'\');" ><button type="button"  class="btn btn-primary">Returned</button></a>';
