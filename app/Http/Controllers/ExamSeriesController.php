...
class ExamSeriesController extends Controller
{
    ...
    public function index()
    {
      if(!checkRole(getUserGrade(2)))
      {
        ...
        $data['sub_active_class']   = 'exam_series';
    ...
    public function getDatatable()
    {
      ...
      if(!checkRole(getUserGrade(2)))
      {
        ...
        $records = array();
            ...
            $records = ExamSeries::select(['image','title',  'is_paid', 'cost', 'validity',  'total_exams','slug', 'id', 'updated_at'])
        ...
        ->addColumn('action', function ($records) {
           ...
           $link_data = '<p id="social-buttons">
                      ...
                      <a href="'.URL_EXAM_SERIES_UPDATE_SERIES.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("update_quizzes").'"  class="btn btn-sm btn-icon btn-success"><i class="fa fa-spinner"></i></a>

                      <a href="'.URL_EXAM_SERIES_EDIT.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("edit").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-pencil"></i></a>
                      ';
        ...
        {
        	return '<strong><a href="'.URL_EXAM_SERIES_UPDATE_SERIES.$records->slug.'">'.$records->title.'</strong></a>';
        })
        ...
        ->editColumn('image', function($records)
        {
          ...
          if($records->image)
            ...
            return '<img src="'.$image_path.'" height="70" width="70" class="img-circle" />';
        })
        ->editColumn('is_paid', function($records)
        {
            return ($records->is_paid) ? '<span class="label bg-warning m-l-xs">'.getPhrase('paid') .'</span>' : '<span class="label bg-success m-l-xs">'.getPhrase('free').'</span>';
        })
