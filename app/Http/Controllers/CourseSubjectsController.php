...
class CourseSubjectsController extends Controller
{
    ...
    public function index($slug='courses')
    {
        ...
        $data['sub_active_class']   = 'allocate_staff';
        $data['slug']               = $slug;
    ...
    public function getDatatable($slug)
    {
        ...
        ->addColumn('action', function ($records) {
               ...
               return '<p id="social-buttons">
                      ...
                      <a href="'.URL_MASTERSETTINGS_COURSE_SUBJECTS_GET_COURSE_SUBJECTS_SHOW.$records->academic_id.'/'.$records->course_id.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("view").'"  class="btn btn-sm btn-icon btn-primary"><i class="fa fa-eye"></i></a>

                      <a href="'.URL_COURSE_SUBJECTS_ADD_STAFF.$records->academic_id.'/'.$records->course_id.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("allocate_staff").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-user"></i></a>

                      <a href="javascript:void(0);" onclick="deleteRecordCustom('.$records->academic_id.','.$records->course_id.');" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("delete").'"  class="btn btn-sm btn-icon btn-danger">
        ...
        ->editColumn('academic_year_title',function($records)
        {
           ...
           if($type=='courses')
           {
            return '<strong><a href="'.URL_MASTERSETTINGS_COURSE_SUBJECTS_ADD.$records->slug.'">'.$records->academic_year_title.'</a></strong>';
           }
            return '<strong><a href="'.URL_COURSE_SUBJECTS_ADD_STAFF.$records->academic_id.'/'.$records->course_id.'">'.$records->academic_year_title.'</a></strong>';
        })
    ...
    public function create()
    {
        $data['record']             = FALSE;
        $data['active_class']       = 'courses';
        $data['sub_active_class']   = 'allocate_subject';
        ...
        $exception_occured = 0;
         ...
         try{
          ...
          if($count)
          {
            ...
            if($request->has('selected_list') && $request->has('total_classes')){
