...
class LibraryMastersController extends Controller
{
    ...
    public function index()
    {
          if(!checkRole(getUserGrade(8)))
      {
        ...
        $data['sub_active_class']   = 'master_type';
    ...
    {
        $records = LibraryMaster::select(['image','title',   
         	'asset_type_id',   'author_id', 'publisher_id', 'total_assets_count', 'total_assets_available', 'total_assets_issued','edition','id','slug']);
       $this->setLibrarySettings();
        ...
        ->addColumn('action', function ($records) {
           ...
           $link_data = '<p id="social-buttons">
                      ...
                      <a href="'.URL_LIBRARY_COLLECTIONS.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("view_items").'"  class="btn btn-sm btn-icon btn-success"><i class="fa fa-paper-plane"></i></a>

                      <a href="'.URL_LIBRARY_MASTERS_EDIT.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("edit").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-pencil"></i></a>

                      <a href="'.URL_LIBRARY_ASSET_DETAILS.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("asset_details").'"  class="btn btn-sm btn-icon btn-default"><i class="fa fa-outdent"></i></a>';
        ...
        ->editColumn('image', function($records){
            ...
            return '<a href="'.URL_LIBRARY_COLLECTIONS.$records->slug.'"><img src="'.$path.'" height="80" width="80" class="img-circle" /></a>';
        })

        ->editColumn('title', function ($records) {
            return '<strong><a href="'.URL_LIBRARY_COLLECTIONS.$records->slug.'">'.$records->title.'</strong></a>
                    <p><strong>'.getPhrase('total').'</strong>:'.$records->total_assets_count.'</p><p><strong>'.getPhrase('available').'</strong>:'.$records->total_assets_available.'</p><p><strong>'.getPhrase('issued').'</strong>:'.$records->total_assets_issued.'</p>';
        })
        ...
        ->removeColumn('id')
        ->removeColumn('slug')
        ->removeColumn('total_assets_count')
        ->removeColumn('total_assets_available')
        ->removeColumn('total_assets_issued')
        ->make();
    }
       ...
       $this->setLibrarySettings();
        ...
        ->addColumn('action', function ($records) {
            ...
            $link_data = '<p id="social-buttons">
                      ...
                      <a href="'.URL_LIBRARY_COLLECTIONS_BARCODE.$records->asset_no.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("barcode").'"  class="btn btn-sm btn-icon btn-success" target = "_blank"><i class="fa fa-barcode"></i></a>';

                      if($records->status=='available'){
                        $link_data .='&nbsp;<a href="javascript:void(0);" onclick="changeStatus(\''.$records->id.'\',\'damaged\');"  title="'.getPhrase("damaged").'"  class="btn btn-sm btn-icon btn-warning"><i class="fa fa-chain-broken"></i></a>';

                          $link_data .='&nbsp;<a href="javascript:void(0);" onclick="changeStatus(\''.$records->id.'\',\'lost\');"  title="'.getPhrase("lost").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-minus-square"></i></a>';
                      }

                            $temp = '';
                            if(checkRole(getUserGrade(8))) {
                                $temp .= '&nbsp;<a href="javascript:void(0);" onclick="deleteRecord(\''.$records->id.'\');" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("delete").'"  class="btn btn-sm btn-icon btn-danger"><i class="fa fa-trash"></i></a>';
                            }
        ...
        ->editColumn('asset_no',function($records){
             
             return '<strong>'.$records->asset_no.'</strong>';
     ...
     catch(Exception $e)
     {
      ...
      DB::rollBack();
       ...
       if(getSetting('show_foreign_key_constraint','module'))
       {
        ...
        flash('oops...!','the_same_prefix_is_already_available', 'warning');
       }
     }
