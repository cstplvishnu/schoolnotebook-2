...
class AcademicsController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['sub_active_class']   = 'acdemics';
    ...
    public function getDatatable()
    {
        DB::statement(DB::raw('set @rownum=0'));
         ...
         $records = Academic::select([ 'id','academic_year_title','academic_start_date','academic_end_date','show_in_list','slug']);
        ...
        ->addColumn('action', function ($records) {
            ...
            return '<p id="social-buttons">
                      ...
                      <a href="'.URL_MASTERSETTINGS_ACADEMICS_COURSES.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("allocate_courses").'"  class="btn btn-sm btn-icon btn-primary"><i class="fa fa-book"></i></a>

                      <a href="'.URL_MASTERSETTINGS_ACADEMICS_EDIT.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("edit").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-pencil"></i></a>
        ...
        {
            return '<strong><a href="'.URL_MASTERSETTINGS_ACADEMICS_COURSES.$records->slug.'">'.$records->academic_year_title.' <span class="text-danger">('.$records->id.')<span>'.'</strong></a>';
        })

          ->editColumn('show_in_list',function($records)
        {
            return ($records->show_in_list==1) ? '<span class="label bg-success m-l-xs">'.getPhrase('yes') .'</span>' : '<span class="label bg-warning m-l-xs">'.getPhrase('no').'</span>';
        })
        ->removeColumn('id')
        ->removeColumn('slug')
        ->make();
