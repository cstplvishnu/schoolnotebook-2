...
class TimetableController extends Controller
{
    ...
    public function viewTimetable()
    {
      $data['active_class']       = 'academic';
      $data['active_class1']       = '';
      $data['sub_active_class']       = 'time_table';
        $data['title']              = getPhrase('timetable');
    ...
    public function updateTimetable(Request $request)
    {
        ...
        if($request->has('current_year'))
         ...
         {
         flash('Oops...!','Please Selcet the details', 'warning');
     ...
     public function printTimetable(Request $request)
     {
      ...
      { 
        ...
        if($role=='student')
        {
            ...
            {
                 flash('Oops...!','Student Roll Number Is Not Generated', 'warning');
       ...
       $data['title']              = getPhrase('timetable');
        ...
        if($user_id)
        {
           ...
           if(!count($allocated_periods->timemaps[1]['timeset'])) {
             flash('Oops...!','Time table is not allocated for you', 'warning');
             return back();
          }
        ...
        if(!count($allocated_periods->timemaps[1]['timeset'])){
          flash('Oops...!','Time table is not created for your class', 'warning');
          return back();
        }
