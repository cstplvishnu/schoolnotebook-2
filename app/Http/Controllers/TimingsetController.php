...
class TimingsetController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['active_class1']      = '';
        $data['sub_active_class']   = 'time_table_timings';
    ...
    public function getDatatable()
    {
        ...
        ->addColumn('action', function ($records) {
         ...
         return '<p id="social-buttons">
                      ...
                      <a href="'.URL_TIMINGSET_EDIT.'/'.$records->slug.'" data-toggle="tooltip" data-placement="auto" title="'.getPhrase("edit").'"  class="btn btn-sm btn-icon btn-info"><i class="fa fa-pencil"></i></a></p>';

        
            })
        
        ->editColumn('name',function($records){
            return '<strong><a href="'.URL_TIMINGSET_EDIT.'/'.$records->slug.'">'.$records->name.'</a></strong>';
        })
        ->removeColumn('id')
        ->removeColumn('slug')
        ->make();
   ...
   public function update(Request $request, $slug)
    {
        if(count($request->start_time_list)<=0)
        {
            ...
            flash('Ooops..!','timingsets_cannot_be_empty', 'warning');
            return back();
